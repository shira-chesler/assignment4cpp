...
Team::Team(Character* leader){
    ...
    if(leader->getHp()==0) throw std::runtime_error("can't add a dead character to team");
    incAliveMembers();
    this->team.fill(nullptr);
...
//copy constructor
Team::Team(Team &tem){
    this->alive_members = tem.alive_members;
...
void Team::attack(Team *enemys){
    ...
    for (size_t i = 0; i < highest_cowboy_position+1; i++)
    {
        if (team.at(i)->getHp()>0 && enemys->stillAlive()>0)
        {
            if (victim->getHp()==0){
    ...
    {
        if (team.at(i)->getHp()>0 && enemys->stillAlive()>0 && victim!=nullptr)
        {
            if (victim->getHp()==0) {
                enemys->decAliveMembers();
                victim=find_closest(enemys);
...
void Team::check_change_leader(){
    if(this->team.at(leader_position)->getHp()>0) return;
    if(this->alive_members==0) return;
    int tmp = __INT_MAX__;
    size_t newlpos = 11;
    for (size_t i = 0; i < highest_cowboy_position+1; i++)
    {
        if(team.at(i)!=nullptr && team.at(i)->getHp()>0){
            int dist = team.at(leader_position)->distance(team.at(i));
            if(dist<tmp){
                tmp = dist;
                newlpos = i;
            }
...
Character* Team::find_closest(Team* enemy){
    ...
    {
        if(enemy->team.at(i)!=nullptr && enemy->team.at(i)->getHp()>0){
            int dist = enemy->team.at(i)->distance(myleader);
            if (dist<tmp)
            {
                tmp = dist;
...
void Team::setTeamAt(Character *newmember, size_t at){
    ...
    if(newmember->getHp()==0) throw std::runtime_error("can't add a dead character to team");
    if(started_game) throw std::runtime_error("cannot add a member while playing game");
    if(alive_members==10) throw std::runtime_error("team is full");
...
void Team::setLeader(size_t nleaderpos){
    //if(this->team.at(leader_position)->getHp()>0) throw std::runtime_error("cannot change leader when the current is alive");
...
//increment alive members
void Team::incAliveMembers(){
    this->alive_members++;
}

//decreases alive members
void Team::decAliveMembers(){
    this->alive_members--;
}
